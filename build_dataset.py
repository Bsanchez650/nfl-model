...
def add_rolling_form(long_df: pd.DataFrame, window: int = 5) -> pd.DataFrame:
    ...
    IMPORTANT: rolling windows are computed across the full team game log, NOT reset
    at season boundaries. This means week 1 of a new season carries over the tail end
    of the previous season's rolling average instead of starting blank -- e.g. a team's
    week-1 avg_pts_for_L5 reflects their last 5 games of the prior season. This matches
    how real prediction markets treat week 1 (informed by last season, not a blank slate)
    and prevents every season-opener from being dropped for missing features.
    """
    ...
    # season-to-date form (all prior games this season). Week 1 has no games yet
    # this season, so fall back to the team's last known L5 form from the tail end
    # of the previous season instead of leaving it NaN.
    long_df["season_win_pct"] = (
        long_df.groupby(["team", "season"])["win"]
        .transform(lambda s: s.shift(1).expanding().mean())
    )
    long_df["season_win_pct"] = long_df["season_win_pct"].fillna(long_df["win_pct_L5"])
